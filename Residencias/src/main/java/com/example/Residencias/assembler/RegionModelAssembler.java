...
import com.example.Residencias.controller.v2.RegionControllerV2;
...
public class RegionModelAssembler implements RepresentationModelAssembler<RegionDTO, EntityModel<RegionDTO>> {
    ...
    public EntityModel<RegionDTO> toModel(RegionDTO region) {
        return EntityModel.of(region,
                linkTo(methodOn(RegionControllerV2.class).porId(region.getId())).withSelfRel(),
                linkTo(methodOn(RegionControllerV2.class).todas()).withRel("regiones")
        );
    }
}
